...
import {changeGem, changeGender, changeMetal} from '../../../products/filterSlice'
import './NavBar.css'
const NavBar = () => {
  ...
  return (
    ...
    <div className='aft' ><Link to="/Login_SignUp/" className='transition-all my-auto hover:scale-110 flex gap-2 text-1xl font-semibold justify-center'><FaRegUser  size={20}/>Profile</Link></div>
      ...
      <div className='products flex flex-col relative'><Link onClick={()=> { dispatch(changeGender("All")); dispatch(changeMetal("All")); dispatch(changeGem("All"))}  } to="/products/"><li className='transition-all hover:scale-105 hover:underline products w-auto'>Products</li></Link>
      <div className=' flex-col w-[100px] justify-center items-center -left-4 flex-wrap colorCust p-2 proOpt'>
      <Link  onClick={()=> { dispatch(changeGender("w")); dispatch(changeMetal("All")); dispatch(changeGem("All"))  }} to="/products/"><li className='flex w-[100px] transition-all hover:scale-105 hover:underline' >Womens</li></Link>
      <Link  onClick={()=> { dispatch(changeGender("m")); dispatch(changeMetal("All")); dispatch(changeGem("All"))  }}  to="/products/"><li className='flex w-[100px] transition-all hover:scale-105 hover:underline'>Mens</li></Link>
